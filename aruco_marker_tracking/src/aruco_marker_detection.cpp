...
ViewPoint_Estimator::ViewPoint_Estimator() 
{
    ...
    nh.param("camera_frame", camera_frame, std::string("openni_depth_frame"));
    ...
    subImage = it.subscribe("/camera/rgb/image_raw", 1, &ViewPoint_Estimator::image_callback, this);
...
void ViewPoint_Estimator::image_callback(const sensor_msgs::ImageConstPtr &original_image)
{
...
bool ViewPoint_Estimator::chessboard_find_pattern(cv::Mat input_image, cv::Mat output_image)
{
...
bool ViewPoint_Estimator::markers_find_pattern(cv::Mat input_image,cv::Mat output_image)
{
...
tf::Transform ViewPoint_Estimator::arucoMarker2Tf(const aruco::Marker &marker) 
{
