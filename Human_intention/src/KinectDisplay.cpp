                                  ...
                                  xn::DepthGenerator& depthGenerator, bool isDepthPass)
{
	...
	for (int i = 0; i < nUsers; ++i)
	{
		// Update targets for hand history objects: This section need to be carefully workout. We need to add some constant point and check the trajectory.
		{
			...
			g_LeftHandPositionHistory.SetTarget(TARGET_HEAD, pt_world, pt_screen);
			g_RightHandPositionHistory.SetTarget(TARGET_HEAD, pt_world, pt_screen);
		}

		sprintf(strLabel, "JointPositionData:%02d", i);
		OutputData::ScopedFileStreamForAppend fs(strLabel);
		...
		if (g_bDrawSkeleton && userGenerator.GetSkeletonCap().IsTracking(aUsers[i]))
		{
			...
			DrawLimb(userGenerator, depthGenerator, aUsers[i], XN_SKEL_LEFT_HIP, XN_SKEL_RIGHT_HIP);
			   ...
			   glEnd();
                           ...
                           if (EnableRightHand) handtrajectory(userGenerator, depthGenerator, aUsers[i], XN_SKEL_RIGHT_HAND, isDepthPass);
			   ...
			   for (int t = 0; t < NumObjectsOfInterest; ++t)
			   {
			   	SetObjectOfInterestSelected(t, false);
			   }

			   if (EnableRightHand)
			   {
				OutputData::ScopedFileStreamForAppend fs2("HandMoving_Right");
				ofstream &hand_file = fs2.GetStream();

				OutputData::ScopedFileStreamForAppend fs3("HandNearObj_Right");
				ofstream &handnear_file = fs3.GetStream();

				bool IsAnyTargetNearRightHand = false;
				...
				{
					SetObjectOfInterestSelected(ObjectNearRightHand, true);
					handnear_file << ObjectNearRightHand + 2;
				}
				else
				{
					handnear_file << 0;
			    ...
			    if (EnableLeftHand)
			    {
				OutputData::ScopedFileStreamForAppend fs2("HandMoving_Left");
				ofstream &hand_file = fs2.GetStream();

				OutputData::ScopedFileStreamForAppend fs3("HandNearObj_Left");
				ofstream &handnear_file = fs3.GetStream();

				bool IsAnyTargetNearLeftHand = false;
				...
				{
					SetObjectOfInterestSelected(ObjectNearLeftHand, true);
					handnear_file << ObjectNearLeftHand + 2;
				}
				else
				{
					handnear_file << 0;
				...
				handnear_file << "\r\n";
				   ...
				   if (g_LeftHandPositionHistory.IsStationary() == false)
				   {
					hand_file << "1";
