...
void AttentionMap::draw_heat_polygon(XnSkeletonJoint eJoint)
{
	const cv::Scalar color(1, 1, 1);

	History *history = 0;
	if (GetHistoryForJoint (eJoint, &history)) {
		if (history && history->IsStationary() == false) {
			XnV3DVector p0 = history->GetCurrentScreenPosition();

			bool bIsNearAnyTargetObject = false;
			for (int k = TARGET_HEAD; k < history->GetNumObjectsOfInterest(); ++k)
			{
				if (history->IsNearTarget(k)) {
					bIsNearAnyTargetObject = true;
					break;
				}
			}
			if (!bIsNearAnyTargetObject)
			{
				poly.clear();
				for (int k = 0; k < history->GetNumObjectsOfInterest() - 1; ++k)
				{
					poly.push_back(cv::Point(p0.X, p0.Y));

					XnPoint3D p = GetObjectOfInterestProjPos(k);
					poly.push_back(cv::Point(p.X, p.Y));

					int nextK = (k + 1) < history->GetNumObjectsOfInterest()? k + 1 : k;
					p = GetObjectOfInterestProjPos(nextK);
					poly.push_back(cv::Point(p.X, p.Y));
				}

				int numTriangles = poly.size() / 3;
				for (int k = 0; k < numTriangles; ++k)
				{
					fillConvexPoly(m_heatmap, poly.data() + 3 * k, 3, color);
				}
			}
		}
	}
...
void AttentionMap::update()
{
	...
	draw_heat_polygon(XN_SKEL_LEFT_HAND);
	draw_heat_polygon(XN_SKEL_RIGHT_HAND);
